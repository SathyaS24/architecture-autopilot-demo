...
export function generateStrategy(
  ...
  // Helper to recursively find .interface.ts files via filesystem analysis
  const findInterfaceFiles = (dir: string): string[] => {
    ...
    for (const file of list) {
      ...
      if (stat && stat.isDirectory()) {
        results.push(...findInterfaceFiles(fullPath));
  ...
  // Find already existing interface files via filesystem analysis
  const existingInterfaces = findInterfaceFiles(resolvedTarget);
  for (const intf of existingInterfaces) {
    if (!extractedInterfaces.includes(intf)) {
      extractedInterfaces.push(intf);
    }
  }
